...
class User:
    ...
    async def get_user_by_email(db: AsyncIOMotorDatabase, email: str):
        user = await db.users.find.one({'email': email})
        if user:
    ...
    async def create_new_user(db: AsyncIOMotorDatabase, data):
        ...
        user = await db.users.find.one({'email': email})
        if user:
            return dict(error='user with enail {} exist'.format(email))
    ...
    @staticmethod
    async def add_friends(db: AsyncIOMotorDatabase, user_id: str, friend_id: str):
    ...
    async def get_user_friends(db: AsyncIOMotorDatabase, user_id: str, limit=20):
        user = await db.users.find_one({'_id': ObjectId(user_id)})
        user_frends = await db.users.find({'_id': {'$in': user['friends']}}).to_list(limit)
        return user_frends
